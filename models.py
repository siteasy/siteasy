import siteasy
from siteasy.utils import get_md_content,render,loadjson,renderMD
from siteasy.settings import global_config,logging,global_context,global_site_map
import os
import json
import copy
...
class BaseView:
    ...
    def get_all_sub_articles(self):
        ...
        for child in self.children:
            if not child.is_cate:
                articles.append({'text':child.text,'url':child.url,'date':child.date,'short_md_content':renderMD(child.short_md_content)})
            else:
    ...
    def apply_md_file(self):
        if self.md_file:
            ...
            self.date = date
            self.update_context({'title':title,'short_md_content':renderMD(self.short_md_content)})
            #if not self.out:
    ...
    def gen_html(self):
        if not self.ext_url:
            ...
            self.update_context({'md_content':renderMD(self.md_content),'short_md_content':renderMD(self.short_md_content)})
...
class BasePlugin:
    ...
    def __init__(self,name):
        self.name = name
        self.config = loadjson(os.path.join('siteasy','plugins',name,'config.json'))
